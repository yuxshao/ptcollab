...
void PxtoneUnitIODevice::set(const mooParams *moo_params, std::shared_ptr<pxtnUnitTone> unit)
{
    m_moo_params = moo_params;
    m_unit = unit;
}

void PxtoneUnitIODevice::stopNote() {
    m_unit->Tone_ZeroLives();
}
...
qint64 PxtoneUnitIODevice::readData(char *data, qint64 maxlen) {
  ...
  while (filled < maxlen) {
    ...
    int result = m_pxtn->moo_tone_sample(m_unit.get(), *m_moo_params, data + filled,
