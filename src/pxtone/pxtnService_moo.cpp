...
bool pxtnService::_moo_ResetVoiceOn(pxtnUnit* p_u, int32_t w) const {
  ...
  std::shared_ptr<const pxtnWoice> p_wc = (Woice_Get(w));

  if (!p_wc) return false;

  p_u->set_woice(p_wc.get());
...
void mooParams::resetVoiceOn(pxtnUnitTone* p_u) const {
  p_u->Tone_Reset_and_2prm(bt_tempo, clock_rate, 0);  //???
}
...
void mooParams::processNonOnEvent(pxtnUnitTone* p_u, EVENTKIND kind,
  ...
  switch (kind) {
    ...
    case EVENTKIND_VOICENO: {
      ...
      /// the key.
      p_u->set_woice((pxtn->Woice_Get(value >= 0 ? value : -value - 1).get())/*,
                     (value >= 0)*/);
      resetVoiceOn(p_u);
    } break;
    ...
    case EVENTKIND_ON: {
      ...
      if (!(p_wc = std::make_shared<const pxtnWoice>(p_u->get_woice()))) break;
      for (int32_t v = 0; v < p_wc->get_voice_num(); v++) {
