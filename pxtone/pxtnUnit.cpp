...
void pxtnUnit::Tone_Sample( bool b_mute_by_unit, int32_t ch_num, int32_t  time_pan_index, int32_t  smooth_smp )
{
	...
	for( int32_t  ch = 0; ch < pxtnMAX_CHANNEL; ch++ )
	{
		...
		for( int32_t v = 0; v < _p_woice->get_voice_num(); v++ )
		{
      /* tone represents configuration (e.g. wave offset) particular voice for this unit */
			pxtnVOICETONE*           p_vt = &_vts                 [ v ];
      /* instance is the actual sample data */
        ...
        /* Bytes: LLRRLLRR, increasing in time, I think. */
				...
				work += *( (short*)&p_vi->p_smp_w[ pos ] ); /* syntax just means read the thing at pos as short */
        ...
        /* scaling filters */
				...
				if( p_vi->env_size ) work = work * p_vt->env_volume / 128; /* ENVELOPE!! */
