...
bool pxtnMaster::io_w_v5(pxtnDescriptor *p_doc, int32_t rough) const {
  ...
  if (!p_doc->w_asfile(&size, sizeof(uint32_t), 1))
    return false;
  if (!p_doc->w_asfile(&bclock, sizeof(int16_t), 1))
    return false;
  if (!p_doc->w_asfile(&bnum, sizeof(int8_t), 1))
    return false;
  if (!p_doc->w_asfile(&btempo, sizeof(float), 1))
    return false;
  if (!p_doc->w_asfile(&clock_repeat, sizeof(int32_t), 1))
    return false;
  if (!p_doc->w_asfile(&clock_last, sizeof(int32_t), 1))
    return false;
...
pxtnERR pxtnMaster::io_r_v5(pxtnDescriptor *p_doc) {
  ...
  if (!p_doc->r(&size, sizeof(uint32_t), 1))
    return pxtnERR_desc_r;
  if (size != 15)
    return pxtnERR_fmt_unknown;

  if (!p_doc->r(&beat_clock, sizeof(int16_t), 1))
    return pxtnERR_desc_r;
  if (!p_doc->r(&beat_num, sizeof(int8_t), 1))
    return pxtnERR_desc_r;
  if (!p_doc->r(&beat_tempo, sizeof(float), 1))
    return pxtnERR_desc_r;
  if (!p_doc->r(&clock_repeat, sizeof(int32_t), 1))
    return pxtnERR_desc_r;
  if (!p_doc->r(&clock_last, sizeof(int32_t), 1))
    return pxtnERR_desc_r;
...
int32_t pxtnMaster::io_r_v5_EventNum(pxtnDescriptor *p_doc) {
  ...
  if (size != 15)
    return 0;
  int8_t buf[15];
  if (!p_doc->r(buf, sizeof(int8_t), 15))
    return 0;
  return 5;
}
...
pxtnERR pxtnMaster::io_r_x4x(pxtnDescriptor *p_doc) {
  ...
  if (!p_doc->r(&size, 4, 1))
    return pxtnERR_desc_r;
  if (!p_doc->r(&mast, sizeof(_x4x_MASTER), 1))
    return pxtnERR_desc_r;
